...
AudioDeviceID obtainDefaultInputDevice()
{
    AudioDeviceID theAnswer = kAudioObjectUnknown;
    UInt32 theSize = sizeof(AudioDeviceID);
    AudioObjectPropertyAddress theAddress;
    ...
    //first be sure that a default device exists
    if (! AudioObjectHasProperty(kAudioObjectSystemObject, &theAddress) )	{
        NSLog(@"Unable to get default audio input device");
        return theAnswer;
    }
    //get the property 'default output device'
    OSStatus theError = AudioObjectGetPropertyData(kAudioObjectSystemObject, &theAddress, 0, NULL, &theSize, &theAnswer);
    if (theError != noErr) {
        NSLog(@"Unable to get output audio input device");
        return theAnswer;
    }
    return theAnswer;
}
...
{
    return obtainDefaultInputDevice();
}
...
{	
	...
	if (! AudioObjectHasProperty(defaultDevID, &theAddress) ) {
		NSLog(@"No volume control for Volume device 0x%0x",defaultDevID);
		return 0.0;
	}
	...
	if ( theError != noErr )	{
		NSLog(@"Unable to read volume for Volume device 0x%0x", defaultDevID);
		return 0.0;
	}
...
+(float)systemMicphone
{
    ...
    defaultDevID = obtainDefaultInputDevice();
    if (defaultDevID == kAudioObjectUnknown) return 0.0;		//device not found: return 0
    
    theAddress.mSelector = kAudioHardwareServiceDeviceProperty_VirtualMasterVolume;//kAudioHardwareServiceDeviceProperty_VirtualMasterVolume;
    theAddress.mScope = kAudioObjectPropertyScopeInput;//kAudioObjectPropertyScopeInput;
    theAddress.mElement = kAudioObjectPropertyElementMaster;
    
    //be sure that the default device has the volume property
    if (! AudioObjectHasProperty(defaultDevID, &theAddress) ) {
        NSLog(@"No input control for Micphone device 0x%0x",defaultDevID);
        return 0.0;
    }
    ...
    theError = AudioObjectGetPropertyData(defaultDevID, &theAddress, 0, NULL, &theSize, &theVolume);
    if ( theError != noErr )	{
        NSLog(@"Unable to read input for Micphone device 0x%0x", defaultDevID);
        return 0.0;
    }
    theVolume = theVolume > 1.0 ? 1.0 : (theVolume < 0.0 ? 0.0 : theVolume);
...
+ (void)setSystemVolume:(float)theVolume
{
	...
	if ( (muteValue = (newValue < THRESHOLD)) )
	{
		...
		if (hasMute)
		{
			...
			{
				canMute = NO;
				NSLog(@"Should mute Volume device 0x%0x but did not success",defaultDevID);
			}
		}
		else canMute = NO;
	}
...
// **** now manage the volume following the what we found ****
	...
	if (! AudioObjectHasProperty(defaultDevID, &theAddress))
	{
		NSLog(@"The Volume device 0x%0x does not have a volume to set", defaultDevID);
		return;
	}
	...
	if (muteValue && hasMute && canMute)
	{
		...
		if (theError != noErr)
		{
			NSLog(@"The Volume device 0x%0x was not muted",defaultDevID);
			return;
		}
	}
	else		//else set it
	{
		...
		if (theError != noErr)
		{
			NSLog(@"The Volume device 0x%0x was unable to set volume", defaultDevID);
		}
...
+(void)setSystemMicphone:(float)theVolume
{
    float						newValue = theVolume;
    AudioObjectPropertyAddress	theAddress;
    AudioDeviceID				defaultDevID;
    ...
    newValue = theVolume > 1.0 ? 1.0 : (theVolume < 0.0 ? 0.0 : theVolume);
    if (newValue != theVolume) {
        NSLog(@"Tentative Input (%5.2f) was out of range; reset to %5.2f", theVolume, newValue);
    }
    ...
    if ( (muteValue = (newValue < THRESHOLD)) )
    {
        ...
        if (hasMute)
        {
            ...
            {
                canMute = NO;
                NSLog(@"Should mute Micphone device 0x%0x but did not success",defaultDevID);
            }
        }
        else canMute = NO;
    }
    ...
    if (! AudioObjectHasProperty(defaultDevID, &theAddress))
    {
        NSLog(@"The Micphone device 0x%0x does not have a volume to set", defaultDevID);
        return;
    }
    ...
    if ( theError!=noErr || !canSetVol )
    {
        NSLog(@"The Micphone of device 0x%0x cannot be set", defaultDevID);
        return;
    }
    ...
    if (muteValue && hasMute && canMute)
    {
        ...
        if (theError != noErr)
        {
            NSLog(@"The Micphone device 0x%0x was not muted",defaultDevID);
            return;
        }
    }
    else		//else set it
    {
        ...
        if (theError != noErr)
        {
            NSLog(@"The Micphone device 0x%0x was unable to set volume", defaultDevID);
        }
        //if device is able to handle muting, maybe it was muted, so unlock it
        if (hasMute && canMute)
        {
    ...
    if (theError != noErr) {
        NSLog(@"Unable to set Micphone for device 0x%0x", defaultDevID);
    }
}
...
+ (void)increaseSystemMicphoneBy:(float)amount
{
    [self setSystemMicphone:[self systemMicphone] + amount];
}

+ (void)decreaseSystemMicphoneBy:(float)amount
{
    [self setSystemMicphone:[self systemMicphone] - amount];
}

+(int)isDefautVolumeMute
{
    AudioDeviceID				defaultDevID = kAudioObjectUnknown;
    AudioObjectPropertyAddress	theAddress;
    OSStatus					theError = noErr;
    ...
    theError = AudioObjectGetPropertyData(defaultDevID, &theAddress, 0, NULL, &size, &muted);
    if (theError != noErr)
    {
        NSLog(@"Error while getting Volume mute status");
...
+(int)isDefautMicphoneMute
{
    ...
    if (theError != noErr)
    {
        NSLog(@"Error while getting micphone mute status");
...
//	OUT:		none
//
+ (void)applyMuteForVolume:(Boolean)m
{
	AudioDeviceID				defaultDevID = kAudioObjectUnknown;
	AudioObjectPropertyAddress	theAddress;
    ...
    if (hasMute)
	{
		...
		{
			theError = AudioObjectSetPropertyData(defaultDevID, &theAddress, 0, NULL, sizeof(muted), &muted);
			if (theError != noErr) NSLog(@"Cannot change mute status of Volume device 0x%0x", defaultDevID);
		}
	}
}

+(void) applyMuteForMicphone:(Boolean)m
{
    AudioDeviceID				defaultDevID = kAudioObjectUnknown;
    AudioObjectPropertyAddress	theAddress;
    Boolean						hasMute, canMute = YES;
    ...
    if (hasMute)
    {
        ...
        {
            theError = AudioObjectSetPropertyData(defaultDevID, &theAddress, 0, NULL, sizeof(muted), &muted);
            if (theError != noErr) NSLog(@"Cannot change mute status of Micphone device 0x%0x", defaultDevID);
        }
    }
}
@end
